 ...
 */
namespace SageKit\Bootstrap4Select\View\Element\Html;
...
class Select extends \Magento\Framework\View\Element\Html\Select {
 ...
 protected function _toHtml() {
  ...
  $html = '<select name="' . $this->getName() . '" id="' . $this->getId() . '" class="form-control' . $this->getClass() . '" title="' . $this->escapeHtml($this->getTitle()) . '" ' . $this->getExtraParams() . '>';
  ...
  foreach ($this->getOptions() as $key => $option) {
   ...
   if (is_array($value)) {
    $html .= '<optgroup label="' . $this->escapeHtml($label) . '" data-optgroup-name="' . $this->escapeHtml($optgroupName) . '">';
